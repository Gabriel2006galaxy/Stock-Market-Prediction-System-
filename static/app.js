...
/* ── PREDICT (SSE-based — never times out) ─────────────── */
async function runPrediction() {
    ...
    // ── Use EventSource (SSE) — keeps connection alive through Render's proxy ──
    const url = `/api/predict/${symbol}?models=${selected.join(",")}`;
    ...
    es.onmessage = (event) => {
        try {
            ...
            if (msg.type === "progress") {
                ...
                fill.style.width   = (msg.pct || 0) + "%";
                return;
            }

            // Either 'done' or 'error' — close the stream
            es.close();
            ...
            if (msg.type === "error") {
                toast("❌ " + msg.msg, false);
                // Show error detail in the status area too
                status.textContent = msg.msg;
                return;
            }
            ...
            if (msg.skipped && msg.skipped.length) {
                toast(`⚠ ${msg.skipped.join(", ")} skipped due to errors`, false);
            }

            show("pred-result");
            window.lastPredictionData     = msg;
            window.lastPredictionSelected = selected.filter(t => msg.predicted[t]);
            renderPrediction(msg, window.lastPredictionSelected, symbol);
    ...
    es.onerror = (e) => {
        ...
        // Check if we got any partial result already rendered
        const currentStatus = status.textContent;
        if (currentStatus && currentStatus !== "Training models… This takes ~60 seconds") {
            toast("Connection interrupted. The server may still be processing — try again.", false);
        } else {
            toast("❌ Could not connect to server. Is it running?", false);
        }
    };
}
...
function renderPrediction(data, selected, symbol) {
    if (!selected || selected.length === 0) {
        toast("No model results to display", false);
        return;
    }
    ...
    setTimeout(() => {
        predActChart = LightweightCharts.createChart(
            document.getElementById("pact-chart"), chartOptions("pact-chart", 260)
        );
    ...
    selected.forEach((tag, idx) => {
        ...
        setTimeout(() => {
            const chart = LightweightCharts.createChart(
                document.getElementById(`pc-${tag}`), chartOptions(`pc-${tag}`, 260)
            );
            const cs = chart.addCandlestickSeries({
                upColor: color, downColor: "#ff4455",
                borderUpColor: color, borderDownColor: "#ff4455",
                    ...
                    <tbody>
                        ${rows.map(r => {
                            ...
                            return `<tr>
                                ...
                                <td class="${cls}">${r.close.toFixed(2)}</td>
...
async function fetchNews(symbol, containerId) {
    ...
    try {
        ...
        news.forEach(n => {
            ...
            html += `
                <div style="background:var(--bg3);padding:14px;border-radius:8px;border:1px solid var(--border);">
...
async function renderMarketNews() {
    ...
    allNews.forEach(n => {
        ...
        html += `
            <div style="background:var(--bg2);padding:18px;border-radius:10px;border:1px solid var(--border);">
